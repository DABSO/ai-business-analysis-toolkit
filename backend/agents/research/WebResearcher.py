...
"""
report_planner_model = ChatVertexAI(model="gemini-2.0-flash-exp", temperature=0, api_key=os.getenv("GOOGLE_GENAI_API_KEY")) 
report_writer_model = ChatVertexAI(model="gemini-2.0-flash-exp", temperature=0, api_key=os.getenv("GOOGLE_GENAI_API_KEY")) 
"""
...
async def generate_report_plan(state: ReportState, config: RunnableConfig):
    ...
    log_function_call("generate_report_plan", 
                     topic=state['topic'],
                     config=config)
...
def generate_queries(state: SectionState, config: RunnableConfig):
    ...
    log_function_call("generate_queries", 
...
def write_section(state: SectionState):
    ...
    log_function_call("write_section",
                     section_name=state['section'].name,
    ...
    # Generate section  
    section_content = report_planner_model.invoke([SystemMessage(content=system_instructions)]+[HumanMessage(content="Generate a report section based on the provided sources.")])
...
def initiate_section_writing(state: ReportState):
    ...
    logger.info(f"Initiating section writing for research sections: {research_sections}")
...
def write_final_sections(state: SectionState):
    ...
    log_function_call("write_final_sections",
                     section_name=state['section'].name,
...
def initiate_final_section_writing(state: ReportState):
    ...
    logger.info(f"Initiating final section writing for: {non_research_sections}")
...
def compile_final_report(state: ReportState):
    ...
    log_function_call("compile_final_report",
